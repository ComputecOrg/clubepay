...
func (h *Handler) GetSpendingStatus(w http.ResponseWriter, r *http.Request) {
	...
	business, err := h.Queries.GetBusinessByOwnerID(ctx, ownerID)
	if err != nil {
	...
	if err != nil {
		...
		newCost, errCreate := h.Queries.CreateMonthlyCost(ctx, repository.CreateMonthlyCostParams{
			BusinessID:         business.ID,
			Month:              pgTypeDate(currentMonth),
			MonthlyBudgetCents: h.Config.MonthlyBudgetCents,
		})
		if errCreate != nil {
			slog.Error("failed to create monthly cost", "error", errCreate)
	...
	if warn {
		...
		if err == nil && recentAlert.SentAt.Valid {
			lastAlertSentAt = &recentAlert.SentAt.Time
		}
	}
